...
class Tree {
    ...
    public ArrayList<Integer> printSums() { // метод для вывода дерева в консоль
        ...
        boolean isRowEmpty = false;
        while (isRowEmpty == false) {
            Stack localStack = new Stack(); // локальный стек для потомков элемента
            isRowEmpty = true;
            ...
            sums.add(summa);
            summa = 0;
            gaps /= 2;// при переходе на следующий уровень расстояние между элементами уменьшается
            while (localStack.isEmpty() == false)
                globalStack.push(localStack.pop()); // перемещаем все элементы из локального стека в глобальный
        }
        return sums;
